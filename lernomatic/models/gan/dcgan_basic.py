...
class DCGGeneratorModule(nn.Module):
    def __init__(self, **kwargs) -> None:
        ...
        # TODO : make this into a collection of distinct modules (and create
        # the network graph in the forward() method) rather than using an
        # nn.Sequential, even if only to make debugging simpler


        # z is input going into covolution
        self.convtranspose1 = nn.ConvTranspose2d(self.zvec_dim,
                            self.num_filters * 8,
                            kernel_size=4,
                            stride=1,
                            padding=0,
        ...
        self.convtranspose2 = nn.ConvTranspose2d(self.num_filters * 8,
                            self.num_filters * 4,
                            kernel_size=4,
                            stride=2,
                            padding=1,
        ...
        self.convtranspose3 = nn.ConvTranspose2d(self.num_filters * 4,
                            self.num_filters * 2,
                            kernel_size=4,
                            stride=2,
                            padding=1,
        ...
        self.convtranspose4 = nn.ConvTranspose2d(self.num_filters * 2,
                            self.num_filters,
                            kernel_size=4,
                            stride=2,
                            padding=1,
        ...
        self.convtranspose5 = nn.ConvTranspose2d(self.num_filters,
                            self.num_channels,
                            kernel_size=4,
                            stride=2,
                            padding=1,
    ...
    def forward(self, X) -> torch.Tensor:
        ...
        # block 2
        out = self.convtranspose2(X)
        out = self.bn2(out)
        out = self.relu2(out)

        # block 3
        out = self.convtranspose3(X)
        out = self.bn3(out)
        out = self.relu3(out)

        # block 4
        out = self.convtranspose4(X)
        out = self.bn4(out)
        out = self.relu4(out)
...
class DCGDiscriminatorModule(nn.Module):
    def __init__(self, **kwargs):
        ...
        # state size (num_filters * 2) x 16 x 16
        self.conv3 = nn.Conv2d(self.num_filters * 2,
                    self.num_filters * 4,
                    kernel_size=4,
                    stride=2,
                    padding=1,
        ...
        # state size (num_filters * 4) x 8 x 8
        self.conv4 = nn.Conv2d(self.num_filters * 4,
                    self.num_filters * 8,
                    kernel_size=4,
                    stride=2,
                    padding=1,
        ...
        # state size (num_filters * 8) x 4 x 4
        self.conv5 = nn.Conv2d(self.num_filters * 8,
                    1,
                    kernel_size=4,
                    stride=1,
                    padding=0,
